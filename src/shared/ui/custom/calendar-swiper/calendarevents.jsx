...
import { eventsData } from "../../../../pages/events/events";

export default function CalendarComponent2({ onFilterChange }) {
      ...
      {selectedDate && (
        <div className="w-full mt-4">
          <h3 className="text-lg font-semibold mb-2 text-gray-800 dark:text-gray-100">
            {t("Руйдодҳо барои рузи")} {selectedDate.toLocaleDateString()}
          </h3>
          {filteredEvents.length > 0 ? (
            <ul className="space-y-2">
              {filteredEvents.map((event) => (
                ...
                >
                  <h4 className="text-md font-medium">{event.eventDescription}</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-300">{event.eventLocation}</p>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-sm text-gray-500 dark:text-gray-400">{t("Дар ин руз ягон руйдод нест")}</p>
          )}
        </div>
      )}
    </div>
